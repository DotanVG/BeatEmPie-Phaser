...
export class MainMenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(cx, 380, pieRow, { fontSize: '44px' }).setOrigin(0.5);

    makeButton(this, cx, 540, '▶  PLAY', () => this.startGame(), { width: 420, height: 100, fontSize: 48 });
  ...
  private buildMuteToggle(): void {
    const label = this.add
      .text(GAME_WIDTH - 40, 40, this.audio.isMuted() ? '🔇 Muted' : '🔊 Sound', {
        fontFamily: 'Trebuchet MS, sans-serif',
        fontSize: '30px',
        color: COLORS.cream,
      })
